    ...
    <div class="app-main flex-column flex-row-fluid" id="kt_app_main">
                    ...
                    <div class="card card-flush">
                        <div class="card-header pt-7">
                            ...
                            <div class="d-flex align-items-center">
                                <button type="button" class="text-center btn btn-primary btn-sm me-3" id="randomAllRtp">Random All RTP</button>
                                <button type="button" class="btn btn-success btn-sm me-3" id="addRtp">+ Add RTP</button>
                            </div>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
                                <table style="width: 100%; font-size:14px" class="table align-middle table-row-dashed fs-6 gy-3" id="table-rtp">
                                    <thead>
                                        <tr class="text-gray-500 text-start fw-bold fs-6 gs-0">
                                            <th style="width: 50px; text-align: center; text-transform:capitalize">Index</th>
        ...
        <div class="modal-dialog">
            <div class="modal-content position-relative">
                ...
                <div id="loadingForm" class="top-0 position-absolute w-100 h-100 d-flex align-items-center justify-content-center d-none" style="z-index: 9999; background: #ffffff21; cursor: wait;">
                ...
                <form id="storeData" enctype="multipart/form-data" action="/rtp" class="needs-validation" novalidate>
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <select name="provider" class="form-control form-control-sm text-uppercase">
                                @foreach (getProvider() as $item)
                                    <option value="{{ $item->provider_name }}" class="text-uppercase">{{ $item->provider_name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="name" class="form-control form-control-sm" placeholder="Nama Game" required>
                            <div class="invalid-feedback"></div>
                        </div>
                        <div class="mb-3">
                            <label for="exampleInputEmail1" class="form-label">Image</label>
                            <input type="file" name="image" class="form-control form-control-sm" id="exampleInputEmail1" aria-describedby="emailHelp" accept="image/png, image/gif, image/jpeg">
                            <div class="invalid-feedback"></div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" name="urutan" class="form-control form-control-sm" placeholder="Urutan Game">
                            <div class="invalid-feedback"></div>
                        </div>
                        <div class="my-4 separator"></div>
                        <button type="button" class="mb-3 text-center d-flex btn btn-primary btn-sm align-items-center" id="randomRtpSingle">
                        ...
                        <div class="mb-3 progress bar-persen">
                            ...
                            <div class="progress-bar bg-animasi {{
                                $p >= 30 && $p <= 40 ? 'bg-danger' :
                                ($p >= 41 && $p <= 60 ? 'bg-warning' :
                                ($p >= 61 && $p <= 100 ? 'bg-success' : ''))
                            }}" role="progressbar" id="progresArea" style="width: {{ $p }}%;" aria-valuenow="{{ $p }}" aria-valuemin="0" aria-valuemax="100">{{ $p }}%</div>
                        </div>
                        <div class="mb-3">
        ...
        $(document).ready(function() {
            ...
            let table = $('#table-rtp').DataTable({
                ...
                drawCallback: () => {
                    ...
                    $(".edit").click(function() {
                        ...
                        if (data.persentase >= 30 && data.persentase <= 40) progresArea.addClass("bg-danger");
                        if (data.persentase >= 41 && data.persentase <= 60) progresArea.addClass("bg-warning");
                        if (data.persentase >= 61 && data.persentase <= 100) progresArea.addClass("bg-success");

                        progresArea.attr("aria-valuenow", data.persentase);
                        progresArea.css({width: data.persentase+"%"});
                    ...
                    $(".delete").click(function() {
                        ...
                        Swal.fire({
                            ...
                            text: 'Mau delete data RTP '+data.name,
                            icon: 'info',
                            showCancelButton: true,
                            confirmButtonColor: "#006ae6",
                            cancelButtonColor: "#f8285a",
                            confirmButtonText: 'Iya delete aja bang...',
                            cancelButtonText: 'eh gak jadi bang...',
                            preConfirm: () => {
                                return new Promise((resolve) => {
                                    ...
                                    fetch("/rtp/"+data.id, {
                                        method: "POST",
                                        body: new FormData(document.getElementById('formDelete'))
                                    }).then(response => response.json()).then(e => {
                                        // Menutup loading dan menampilkan pemberitahuan berhasil jika sukses
                                        Swal.close();
                                        Swal.fire('Success!', 'Data RTP '+data.name+" berhasil di delete", 'success');
                                        table.draw();
                                        resolve(true);
                                    }).catch(error => {
                                        ...
                                        Swal.close();
                                        Swal.fire('Failed!', 'Error: Ada kesalahan', 'error');
                                        resolve(false);
                                    });
                                });
                            }
            ...
            formGlobal.submit(function(e) {
                ...
                $.ajax({
                    ...
                    error: (e) => {
                        loadingForm.addClass("d-none");
                        const { provider, name, url, urutan, image } = e.responseJSON;
                        if (provider) formGlobal.find('[name="provider"]').addClass("is-invalid").parent().find(".invalid-feedback").text(provider);
                        if (name) formGlobal.find('[name="name"]').addClass("is-invalid").parent().find(".invalid-feedback").text(name);
                        if (url) formGlobal.find('[name="url"]').addClass("is-invalid").parent().find(".invalid-feedback").text(url);
                        if (urutan) formGlobal.find('[name="urutan"]').addClass("is-invalid").parent().find(".invalid-feedback").text(urutan);
                        if (image) formGlobal.find('[name="image"]').addClass("is-invalid").parent().find(".invalid-feedback").text(image);
                    }
                });
            });
                ...
                Swal.fire({
                    ...
                    preConfirm: () => {
                        return new Promise((resolve) => {
                            ...
                            // Menggunakan Fetch API untuk mengirim permintaan asinkron
                            fetch("/rtp/randomAll").then(response => response.json()).then(data => {
                                // Menutup loading dan menampilkan pemberitahuan berhasil jika sukses
                                Swal.close();
                                Swal.fire('Success!', 'Your pattern has been updated', 'success');
                                table.draw();
                                resolve(true);
                            }).catch(error => {
                                console.error('Error:', error);
